...
import { GEMINI_API_KEY } from "../../client/config";
 ...
 */
export const getConceptsFromGemini = async (text) => {
  try {
    const prompt = ` 
      Extract the most important concepts and topics from this note content. 
      Return only a plain JSON array like: ["Concept A", "Topic B", "Term C"]. 
 ...
 */
export const getGeminiInsights = async (topic) => {
  try {
    const prompt = `
 ...
 */
export const askGeminiQuestion = async (topic, question) => {
  try {
    const prompt = `
